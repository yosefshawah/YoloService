...
async def main() -> None:
    ...
    async with connection:
        ...
        queue = await channel.declare_queue("analytics.events", durable=True)
        # Track multiple event types; for now images.processed
        await queue.bind(exchange, routing_key="images.processed")
        print(" [*] Analytics consumer waiting on 'images.processed'")
